// MAIN BACKEND (V81 - ROLLED BACK TO FAST FETCH)
...
exports.handler = async function(event, context) {
    ...
    try {
        ...
        const now = new Date();
        // ROLLBACK: Only fetch from 1st of month to keep payload small and fast
        ...
        // Weekly Calc (Current Month Only)
        const weekStr = new Date();
        weekStr.setDate(weekStr.getDate() - ((weekStr.getDay() + 6) % 7)); 
        const weekStartIso = weekStr.toISOString().split('T')[0];

        let wStats = { month: { b: 0, t: 0 }, week: { b: 0, t: 0 } };
        ...
        const responseData = {
            ...
            weekly: { 
                ...
                // Last Week is 0 because we don't fetch last month data to save speed
